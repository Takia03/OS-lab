...
void pgflt_handler(void)
{
    ...
    if ((errno & 0x3) == 0x3) {
        ...
        if (pte_entry & PTE_COW) {
            ...
            // Allocate a new page
            new_page = alloc_page(cur_pid, fault_va, PTE_W | PTE_U | PTE_P);
            if(new_page == NULL){
                KERN_PANIC("Copy-on-Write page allocation failed: va = %p\n", fault_va);
            }

            // Get the physical address of the new page
            pa = (unsigned int)new_page;

            // Copy the contennts from the originam page to the new page
            memcpy((void *)new_page, (void *)(fault_va & ~0xFFF), PAGESIZE);

            // Update the page table entry to point to the new writable page
            set_ptbl_entry_by_va(cur_pid, fault_va, pa | PTE_W | PTE_U | PTE_P);
